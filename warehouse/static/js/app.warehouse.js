  ...
  $scope.createInvoice = function(id, from, to) {
    for (var i = 0; i < $scope.invoiceData.length; i++) {
      if ($scope.invoiceData[i].pk == id) {
        // $scope.fromDate = $scope.frmDate.setDate($scope.frmDate.getDate() + parseInt(1))
        console.log($scope.frmDate.setDate($scope.frmDate.getDate() + 1) ,'hhhhhhhhhhhhhhhhhhhhhh');
        var dayDif = (($scope.toDate - $scope.frmDate)  / 1000 / 60 / 60 / 24)+1;
